...
- **UI library:** Fluent UI v9 (`@fluentui/react-components`, `^9.46.0`) is the React component library going forward — wraps the `BrandedHeader` rendered into the customizer's Top placeholder as of Iteration 2b (2026-05-24). Fluent UI v8 (`@fluentui/react`) is still in `devDependencies` for compatibility with SPFx-shipped types but new components should not import it.
...
- **The Application Customizer has exactly TWO global side effects: CSS injection into `<head>` (the `<style id="phil-brand">` tag), and a React tree rendered into the SharePoint `PlaceholderName.Top` slot (the `BrandedHeader` component, wrapped in a `FluentProvider`).** Both were authorized by an explicit design conversation — the CSS side effect by the original brand work, and the React render by D026 in the `partner-exchange-provisioning` repo (Iteration 2b, 2026-05-24). Any further global behavior on top of these two requires another design conversation, not a quiet addition.
...
- **Diagnostic banner:** Removed — `onInit()` invoked only `injectBrandStyles()` at the end of Prompt 4. (As of Iteration 2b, `onInit()` now also wires up the Top-placeholder render — see "Iteration 2b" below.) The original red banner remains historical; see "Known gotchas" for the full record.
