...
// dependency, so they unit-test in isolation). Permissive on input (accept
// `unknown`, handle every shape we've observed), conservative on output (return
// '' / undefined rather than something that renders broken). They warn ONLY when
// input is present but malformed — never for a legitimately empty value.
//
...
//   2. "Reserved attachment" form (15 Practices): { fileName, originalImageName }
//      — no URL in the blob; the real URL lives in the item's AttachmentFiles
//      entry whose FileName matches `fileName`.
// Tries the inline URL first, then the AttachmentFiles lookup. Returns undefined
// when no usable URL can be derived (caller falls back to styled title text).
